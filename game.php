      ...
      <div class="collapse navbar-collapse" id="navbarSupportedContent">
        <ul class="navbar-nav mr-auto">
          ...
          <li class="nav-item active">
            <a class="nav-link" href="game.php">Games <span class="sr-only">(current)</span></a>
          </li>
          <li class="nav-item">
            <a class="nav-link " href="help.php">Help</a>
          </li>
          <p>
       <div id="e">  <form class="form-inline my-2 my-lg-0" action="game.php" method="get">
...
$sql = "SELECT * FROM games WHERE title LIKE  '%$searchWord%'";
...
while ($item = $result->fetch_array()) {
  if (($counter % $cols)==1) {
 echo '<div class="'.$row_class.'">';}$images ="apps/".$item['images'];
   echo '<div class="'.$col_class.'"><img height="268px" width="210px"src="'.$images.'" alt="'.$item['title'].'"/><br><p>'.$item['title'].'<br><a href="'.$item['download_link'].'">Download Torrent</a></p></div>';
  if (($counter % $cols) ==0) {
    echo '</div>';
      }
      $counter++;

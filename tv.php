      ...
      <div class="collapse navbar-collapse" id="navbarSupportedContent">
        <ul class="navbar-nav mr-auto">
          ...
          <li class="nav-item active">
            <a class="nav-link" href="tv.php">Tv-show <span class="sr-only">(current)</span></a>
          </li>
          <li class="nav-item">
          ...
          <p>
         <div id="e"> <form class="form-inline my-2 my-lg-0" action="tv.php" method="get">
...
$sql = "SELECT * FROM movies_seasons ORDER BY title ASC";

if (isset($_GET['term'])) {
$searchWord = $_GET['term'];

$sql = "SELECT * FROM movies_seasons WHERE title LIKE  '%$searchWord%'";
...
while ($item = $result->fetch_array()) {
 ...
 $images ="tv/".$item['images'];
  echo '<div class="'.$col_class.'"><a href="episodes.php?showId='.$item['id'].'"><img height="268px" width="182px"src="'.$images.'" alt="'.$item['title'].'"/></a><br><p>'.$item['title'].'<br/>Rating: '.$item['ratings'].'<br></p></div>';
  if (($counter % $cols) ==0) {
    echo '</div>';
      }
      $counter++;

  ...
  <nav class="navbar navbar-dark bg-dark">
    ...
    <a class="nav-link" href="movies.php">Movies <span class="sr-only">(current)</span></a>
          ...
          <li class="nav-item">
           ...
           <p>
          <div id="e"><form class="form-inline my-2 my-lg-0" action="movies.php" method="get">
...
$sql = "SELECT * FROM movies WHERE title LIKE  '%$searchWord%'";
...
while ($item = $result->fetch_array()) {
 ...
 $images ="images/".$item['images'];
  echo '<div class="'.$col_class.'"><img height="268px" width="182px"src="'.$images.'" alt="'.$item['title'].'"/><br><p>'.$item['title']." (".$item['year_released'].')<br/>Genre: '.$item['genre'].'<br/>Rating: '.$item['ratings'].'<br><a href="'.$item['download_link'].'">Download Torrent</a></p></div>';
  if (($counter % $cols) ==0) {
    echo '</div>';
      }
      $counter++;
